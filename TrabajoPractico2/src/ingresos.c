...
int ingresarFecha(eFecha* resultado)
{
	...
	ingresarIntConRango(&fecha.mes,"Ingrese mes en numero", "ERROR, ingrese mes valido\n",1, 12);
	if(fecha.mes==1 || fecha.mes==3 || fecha.mes==5 || fecha.mes==7 || fecha.mes==8 || fecha.mes==10 || fecha.mes==12)
	{
		ingresarIntConRango(&fecha.dia,"Ingrese dia", "ERROR, ingrese dia valido\n",1, 31);
	}
	else
	{
...
int ingresarShortConRango(short* resultado, char* mensaje, char* mensajeError, short minimo, short maximo)
{
	...
	do
	{
		if(resultado!=NULL && mensaje!=NULL && mensajeError!=NULL && minimo<maximo)
		{
			printf("%s",mensaje);
			fflush(stdin);
			verificacionScanf=scanf("%hd",&numero);
			if(verificacionScanf==1)
			{
				if(numero>=minimo && numero<=maximo)
				{
